...
async function createWsServer() {
  ...
  const io = new socket_io.Server(httpSever, {
...
electron.app.setPath("userData", path.join(electron.app.getPath("appData"), "K-Desk"));
function createWindow() {
  const ses = electron.session.fromPartition("persist:kdesk");
  const mainWindow = new electron.BrowserWindow({ ...utils.defaultBrowserWindowOptions, session: ses });
...
electron.app.whenReady().then(async () => {
  utils.electronApp.setAppUserModelId("com.kiosk-seat-sample");
  electron.app.on("browser-window-created", (_, window) => {
    utils.optimizer.watchWindowShortcuts(window);
  });
  electron.ipcMain.on("ping", () => console.log("pong"));
  electron.ipcMain.handle("get-db-schema", async () => {
    try {
  ...
  electron.ipcMain.handle("get-posts", async () => {
    try {
  ...
  electron.ipcMain.handle("getAllUserCount", async () => {
    try {
  ...
  electron.ipcMain.handle("get-table-list", async () => {
    try {
  ...
  electron.ipcMain.handle("get-table-data", async (event, tableName, limit = 100) => {
    try {
